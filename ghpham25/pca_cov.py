...
class PCA_COV:
    ...
    def __init__(self, data):
        ...
        # instance variables needed to undo normalization
    ...
    def covariance_matrix(self, data):
        ...
        # mean of each column (mean of var across rows)
    ...
    def compute_prop_var(self, e_vals):
        ...
        '''
        prop_var = np.copy(e_vals)
        sum = np.sum(e_vals).squeeze()
        for i in range(e_vals.shape[0]):
            prop_var[i] = e_vals[i]/sum
    ...
    def compute_cum_var(self, prop_var):
        ...
        '''
        cum_var = prop_var.copy()
        for i in range(len(cum_var)):
            for j in range(i):
                cum_var[i] += prop_var[j]
        return cum_var
    ...
    def pca(self, vars, normalize=False):
        ...
        '''
        vars_data = np.array(self.data.loc[:, vars])
        self.vars = vars
        if (normalize):
            # normalize data
            self.normalized = True
            vars_data_range = np.max(vars_data, axis=0) - \
                np.min(vars_data, axis=0)
            vars_data_norm = (
                vars_data - np.min(vars_data, axis=0))/vars_data_range

            self.data_min = np.min(vars_data, axis=0)
            self.data_range = vars_data_range

            self.A = vars_data_norm
        ...
        # need to sort e_vals and sort e_vecs by e_vals
        sort_inds = np.argsort(e_vals)[::-1]
        self.e_vals = e_vals[sort_inds]
        self.e_vecs = e_vecs[:, sort_inds]
    ...
    def elbow_plot(self, num_pcs_to_keep=None):
        ...
        if (num_pcs_to_keep is None):
            x_axis = np.arange(1,  len(list(self.cum_var)) + 1)
            y_axis = self.cum_var
            plt.plot(x_axis, y_axis, marker = "o", markersize = 7)
            # plt.xticks(np.arange(1,  len(list(self.cum_var)) + 1))

        else:
            x_axis = [i for i in range(1, num_pcs_to_keep + 1)]
            y_axis = self.cum_var[:num_pcs_to_keep]
            plt.plot(x_axis, y_axis, marker = "o", markersize = 7)
    ...
    def pca_then_project_back(self, top_k):
        ...
        '''
        pcs_to_keep = [i for i in range(top_k)]
        pca_prj = self.pca_project(pcs_to_keep)
        p_hat = self.e_vecs[:, :top_k] 

        Ares = pca_prj @ p_hat.T + np.mean(self.A, axis=0)
        if (self.normalized):
            Ares= Ares * self.data_range + self.data_min
        return Ares 
